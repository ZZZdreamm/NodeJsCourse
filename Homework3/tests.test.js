const {Product, calculateDiscountedPrice, calculateTotalPrice} = require('./PureFunctions.js');
const {Person, Student, getFullName, filterUniqueWords, getAverageGrade } = require('./FunctionCompositions.js');
...
describe("Pure Functions", () => {
    describe("CalculateDiscountedPrice", () => {
        it("Normal case", () => {
            const productsArray = [new Product("apple", 100), new Product("banana", 200)];
            const discountsArray = [0.1, 0.2];
            const value = calculateDiscountedPrice(productsArray, discountsArray);
            expect(value).toEqual([{name: "apple", price: 90}, {name: "banana", price: 160}]);
            expect(value).not.toBe(productsArray);
            expect(productsArray).toEqual([new Product("apple", 100), new Product("banana", 200)]);
            expect(discountsArray).toEqual([0.1, 0.2]);
        })
        ...
        it("One product", () => {
            ...
            const discountsArray = [0.1];
            const value = calculateDiscountedPrice(productsArray, discountsArray);
            expect(value).toEqual([{name: "apple", price: 90}]);
        })

        it("One product with 0 discount", () => {
            const productsArray = [new Product("apple", 100)];
            const discountsArray = [0];
            const value = calculateDiscountedPrice(productsArray, discountsArray);
...
describe("Higher Order Components", () => {
    ...
    describe("RepeatFunction", () => {
        it("Normal case", () => {
            let counter = 0;
            repeatFunction(() => {counter++}, 5)();
            expect(counter).toEqual(5);
        })

        it("Zero repetitions", () => {
            let counter = 0;
            repeatFunction(() => {counter++}, 0)();
        ...
        // it("Negative number", () => {
        //     let counter = 0;
        //     repeatFunction(() => {counter++}, -1)();
        //     expect(counter).toEqual(Infinity);
        // })
